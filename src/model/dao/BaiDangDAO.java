...
public class BaiDangDAO {
	...
	public ArrayList<BaiDang> layDuLieuBaiDangBangCode(String maThanhVien) {
		...
		String sql = String.format(
				"select * from BaiDang where MaThanhVien = '%s'", maThanhVien);
		ResultSet rs = null;
		try {
	...
	public ArrayList<BaiDang> danhSachBaiDangCodeTrangThai(String trangThai) {
		Connection con = DBConnect.getConnection();
		String sql = String.format(
				"select * from BaiDang where TrangThai = N'%s'", trangThai);
		ResultSet rs = null;
		try {
			Statement stmt = con.createStatement();
			rs = stmt.executeQuery(sql);
	...
	public BaiDang chiTietBaiDang(String maBaiDang) {
		Connection con = DBConnect.getConnection();
		String sql = String
				.format("select * from ChiTietBaiDang  join BaiDang on BaiDang.MaBaiDang = ChiTietBaiDang.MaBaiDang join LoaiBaiDang on LoaiBaiDang.MaLoaiBaiDang = BaiDang.MaLoaiBaiDang where ChiTietBaiDang.MaBaiDang = '%s'",
						maBaiDang);
		ResultSet rs = null;
		try {
			Statement stmt = con.createStatement();
	...
	public boolean capNhatTrangThaiBaiDang(String trangThai, String maBaiDang) {
		Connection con = DBConnect.getConnection();
		String sql = String.format(
				"update BaiDang set TrangThai = N'%s' where MaBaiDang = '%s'",
				trangThai, maBaiDang);
		try {
			Statement stmt = con.createStatement();
	...
	public String themBaiDang(String maLoaiBaiDang, String maThanhVien,
			...
			String trangThai, String gia, String dienTich, String donVi,
			String hinhAnh, String diaChi, String moTa) {
		...
		try {
			PreparedStatement p1 = con
					.prepareStatement(
							"insert into BaiDang(MaLoaiBaiDang,MaThanhVien,MaHuong,MaDanhMuc,MaTinh,MaQuanHuyen,MaPhuongXa,TenBaiDang,TrangThai,MoTa) values(?,?,?,?,?,?,?,?,?,?)",
			...
			if (rs.next()) {
				...
				String check = themChiTietBaiDang(maBaiDangCuoi, gia, dienTich,
						hinhAnh, diaChi);
	...
	// them moi bai dang: BaiDang - ChiTietBaiDang:
	public String themChiTietBaiDang(String maBaiDang, String gia,
			String dienTich, String hinhAnh, String diaChi) {
		...
		try {
			PreparedStatement p1 = con
					.prepareStatement("insert into ChiTietBaiDang(MaBaiDang,Gia,DienTich,HinhAnh,DiaChi) values(?,?,?,?,?)");
	...
	public ArrayList<BaiDang> layDanhSachTimKiemBaiDang(String maLoaiBaiDang,String maTinh,String maQuanHuyen){
		Connection con = DBConnect.getConnection();		
		String sql = String.format("select * from BaiDang join ChiTietBaiDang on BaiDang.MaBaiDang = ChiTietBaiDang.MaBaiDang   join LoaiBaiDang on LoaiBaiDang.MaLoaiBaiDang = BaiDang.MaLoaiBaiDang join Tinh on Tinh.MaTinh = BaiDang.MaTinh join QuanHuyen on QuanHuyen.MaQuanHuyen = BaiDang.MaQuanHuyen where (TrangThai = N'Đang rao' or TrangThai = N'Kết thúc rao') and ( LoaiBaiDang.MaLoaiBaiDang='%s' or Tinh.MaTinh = '%s' or QuanHuyen.MaQuanHuyen ='%s')",maLoaiBaiDang,maTinh,maQuanHuyen);//or NgayBatDauSuDung = '%s' or GioBatDauSuDung = '%s'
		// nho la: DichVu. (cham cham cham cham!!!!!)
		// tim kiem kieu du lieu cac fields trong database phai cung kieu.
		// check database truoc roi vao java.
		// xem cau tim kiem. makh phai int. hoac ngay.; de bo cung kieu
		ResultSet rs = null;
		try {
			Statement stmt = con.createStatement();
			rs = stmt.executeQuery(sql);
	...
	public static void main(String[] args) {
		...
		// b.themBaiDang( "LBD01", "1", "H01", "DM01", "T01", "Q01", "PX01",
		// "abc", "Trang thai ne", "2302322", "12m2", "chiec", "images/abc.jpg",
		// "Qtri", "mo ta abc");
		ArrayList<BaiDang> list = b.layDanhSachTimKiemBaiDang("LBD01","T01","Q04");
